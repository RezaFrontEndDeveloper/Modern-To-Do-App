...
export default function Input() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center border gap-2 bg-white border-gray-500/30 h-12 max-w-md w-full rounded-full overflow-hidden">
      <input
        value={inputFild}
        onChange={(e) => setInputFild(e.target.value)}
        type="text"
        placeholder="enter"
        className="w-full  h-full pl-6 outline-none text-sm placeholder-gray-900 text-black"
        required
      />
